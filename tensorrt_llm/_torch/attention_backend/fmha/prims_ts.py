...
class PrimsTSFmha(PhasedFmha):
    ...
    def __init__(self, attn: "TrtllmAttention") -> None:
        ...
        self._context_wrapper: Optional["BatchPrefillPagedTSWrapper"] = None
        self._context_wrapper_plan_key: Optional[tuple[object, ...]] = None
        self._decode_wrapper: Optional["BatchDecodePagedTSWrapper"] = None
        self._decode_wrapper_plan_key: Optional[tuple[object, ...]] = None
        ...
        self._decode_workspace_size_key: Optional[tuple[object, ...]] = None
        self._decode_workspace_size_bytes = 0
        self._decode_workspace_offset_bytes: Optional[int] = None
        self._decode_workspace_required_bytes = 0
        self._mla_workspace_size_key: Optional[tuple[object, ...]] = None
        self._mla_workspace_size_bytes = 0
    ...
    def _get_static_max_kv_len(
        ...
        *,
        ...
        configured_max = getattr(meta, "max_seq_len", None)
    ...
    def _get_b1_context_support_key(
        ...
        if (
            ...
            or num_heads // num_kv_heads > self.MAX_GQA_RATIO
    ...
    def _get_cache_torch_dtype(meta: "TrtllmAttentionMetadata") -> Optional[torch.dtype]:
        ...
        if isinstance(cache_dtype, torch.dtype):
            return cache_dtype
        return _CACHE_TORCH_DTYPES.get(cache_dtype)
    ...
    def _get_or_plan_context_wrapper(
        ...
        *,
        ...
        wrapper = self._context_wrapper
        if wrapper is not None:
            wrapper.reset_workspace_buffer(workspace_buffer)
            if plan_key == self._context_wrapper_plan_key:
                return wrapper
        if torch.cuda.is_current_stream_capturing():
        ...
        self._context_wrapper_plan_key = plan_key
        return wrapper
    ...
    def _get_or_plan_decode_wrapper(
        ...
        *,
        ...
        self._decode_wrapper_plan_key = plan_key
        return wrapper
    ...
    def prepare_workspace(
        ...
        if self.attn.is_mla_enable:
            workspace_size_key = (
                ...
                seq_len_q,
                q.dtype,
                forward_args.output.dtype,
                mask_type,
            )
            if workspace_size_key != self._mla_workspace_size_key:
                self._mla_workspace_size_bytes = _get_prims_mla_workspace_size(
                    batch_size,
                    self.attn.num_heads,
                    int(self.attn.kv_lora_rank),
                    int(self.attn.qk_rope_head_dim),
                ...
                self._mla_workspace_size_key = workspace_size_key
            required_bytes = self._mla_workspace_size_bytes
        else:
            workspace_size_key = (
                q.device,
                batch_size,
            ...
            if workspace_size_key != self._decode_workspace_size_key:
                self._decode_workspace_size_bytes = _get_prims_decode_workspace_size(
                    batch_size,
                    self.attn.num_heads,
                    self.attn.num_kv_heads,
                    self.attn.head_dim,
                ...
                self._decode_workspace_size_key = workspace_size_key
            required_bytes = self._decode_workspace_size_bytes
        ...
        if self.attn.is_mla_enable:
            ...
            if required_bytes <= generation_fmha_workspace_bytes:
                ...
                # slab. Large PrimTS plans use an aligned tail after every
                # live preprocessing region instead of failing after dispatch.
                self._decode_workspace_offset_bytes = _align_workspace_offset(required_thop_bytes)
                required_workspace_bytes = self._decode_workspace_offset_bytes + required_bytes
        current_workspace_bytes = workspace.numel() * workspace.element_size()
